...
# Add a dummy baseline to bypass regression behavior when disabled
class ZeroRegression(torch.nn.Module):
    def __init__(self, out_channels: int, height: int, width: int, device: torch.device):
        super().__init__()
    ...
    def forward(
        ...
        augment_labels=None,   # accept and ignore
        **kwargs               # future-proof
    ):
        # img_lr: (B, Cin, H, W); return zeros (B, Cout, H, W)
        B = img_lr.shape[0]
        return img_lr.new_zeros(B, self.out_channels, self.height, self.width)
...
def main(cfg: DictConfig) -> None:
    ...
    # If regression is disabled or unavailable, inject ZeroRegression
    if (not cfg.model.hr_mean_conditioning) or (regression_net is None):
        regression_net = ZeroRegression(
            out_channels=img_out_channels,
            height=img_shape[0],
            width=img_shape[1],
            device=dist.device,
        ).to(dist.device).eval()
        logger0.info("Using ZeroRegression baseline (no regression conditioning).")
